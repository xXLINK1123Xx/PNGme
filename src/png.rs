...
impl Display for PngError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PngError::InvalidHeaderError => write!(f, "Invalid PNG header detected"),
            PngError::CorruptedFileError => write!(f, "Invalid chunk detected"),
            PngError::ChunkNotFoundError(name) => write!(f, "Chunk with type \"{}\" was not found", name)
        }
    }
}
